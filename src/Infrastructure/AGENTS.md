...
| `Migrations/` | EF Core migrations for `ApplicationDbContext`, generated via `dotnet ef migrations add`. Currently 7 migrations from `InitialMigration` (2026-03-10) through `DeleteRating` (2026-05-11), plus `ApplicationDbContextModelSnapshot.cs`. Do not hand-edit `*.Designer.cs` or the snapshot — regenerate via the CLI (see "Working In This Directory" below). Not enumerated individually here; read a specific migration file directly if you need to know what a schema change did. |
