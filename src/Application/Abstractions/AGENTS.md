...
| `Authentication/IUserContext.cs` | `IUserContext { Guid UserId }` — contract for reading the current authenticated user's id (implemented in `Infrastructure`/`Web.Api` from the HTTP context / claims). Not currently referenced by any handler in this layer (handlers that need the acting user take `UserId` as an explicit command parameter instead, e.g. `CreateSubmissionCommand.UserId`). |
