...
|------|-------------|
| **admin/** | |
| `admin/admin-shared.tsx` | The admin "kit": `AdminLayout` (sidebar + breadcrumb header chrome; takes no props and renders an `<Outlet/>` — the header reads title/subtitle/actions via `useAdminPage`, and only `layout/app-shell.tsx` may mount it, which is also where the `AdminPageProvider` lives), `AdminTable<T>` (generic searchable table with empty state), `AdminTableRow`, `AdminTableIconCell` (icon-or-initial + title + "Удалено" badge), `AdminTableActions` (edit/delete-with-confirm/restore icon buttons), `AdminTableTextBadge`, and `AdminModal` (simple centered form-modal, not Radix Dialog — hand-rolled overlay + `<Card>`). Every `pages/admin/*` page is built from these primitives |
| **auth/** | |
...
| `auth/auth-session-listener.tsx` | `AuthSessionListener` — renders nothing; mounted once inside `BrowserRouter` in `App.tsx`, listens for the `auth:logout` window event dispatched by `@/utils/auth`'s `logout()` and calls `navigate("/login", { replace: true })`. This is what lets code outside the router tree (e.g. the axios interceptor in `api.ts`) trigger a client-side redirect without a hard `window.location` navigation |
| **dashboard/** | |
| `dashboard/dashboard-content.tsx` | `DashboardContent({ forms, deleteForm?, isAdmin })` — the shared card grid of forms; admin cards get delete (with `AlertDialog` confirm) + stats-link buttons, student cards get a "Пройти опрос" (take survey) button linking to `/form/:id`. `requiredFilters` badges are rendered through `@/utils/filter-fields`'s `FILTER_FIELD_LABELS` (falling back to the raw key) so students/employers/staff see e.g. "Категория персонала" instead of the raw `EmployeeCategory` enum name |
...
| **layout/** | |
| `layout/app-shell.tsx` | `AppShell` — the single layout route for every sidebar-bearing screen, and the only place `AdminLayout` is mounted, so navigating between those screens re-renders the `<Outlet/>` and leaves the sidebar alone. Owns the `AdminPageProvider`, placed above the role branch so `useAdminPageConfig` works for any page under the shell; renders the page without chrome for non-admins (`/dashboard` is the one shell route they can reach). Guarded by `layout/app-shell.test.tsx` |
| `layout/app-sidebar.tsx` | `AppSidebar` — the admin nav: hardcoded `data.navMain` tree (Дашборд / Справочники / Настройки sections with hrefs to the `/admin/*` routes), logo linking to `/dashboard`, footer with `ModeToggle` + logout button (`onClick={logout}` from `@/utils/auth` — client-side only, see `auth/auth-session-listener.tsx`) |
| `layout/nav-main.tsx` | `NavMain` — renders the collapsible sidebar sections from `AppSidebar`'s `data.navMain`; highlights the active route via `useLocation()`. Group open/closed state is controlled and mirrored into `sessionStorage["sidebar:nav-groups"]` so it survives a reload or deep link; the group owning the current route auto-expands, but one the user collapsed by hand stays collapsed. Covered by `layout/nav-main.test.tsx` |
...
| **survey/** | |
| `survey/context-selector.tsx` | `ContextSelector({ requiredFilters, onChange })` — renders the "education form / department / discipline / teacher / employee category" pickers a student fills in before/while taking a survey, conditionally shown per `form.requiredFilters`; loads dictionaries on demand and cross-filters department↔discipline via `getLinkedFilterOptions`. `employeeCategory` is a fixed АУП/ППС/УВП/ПОП `<Select>`, not a loaded dictionary. |
