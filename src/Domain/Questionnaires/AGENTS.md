...
| `Forms/QuestionType.cs` | `[Flags] enum QuestionType { Text = 1, Number = 2, SingleChoice = 4, WeightedRating = 6 }`. **Gotcha**: despite the `[Flags]` attribute, the values are not disjoint bit flags — `WeightedRating = 6` overlaps `Number | SingleChoice` (2|4). This enum is used as a single discrete tag, not as a combinable bitmask; don't write code that does bitwise `HasFlag`/OR logic on it expecting independent bits. (`MultipleChoice = 3` was removed — the domain never stored option lists for it, and it was unused/unreachable from the admin form builder.) |
