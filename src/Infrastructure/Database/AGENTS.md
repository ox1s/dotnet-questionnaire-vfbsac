...
| `DbInitializer.cs` | `DbInitializer(IServiceProvider, ILogger<DbInitializer>, DemoDataGenerator, IHostEnvironment)`. `InitializeAsync()` creates a DI scope, runs `context.Database.MigrateAsync()`, then returns early without seeding when `IHostEnvironment.IsProduction()` is true. Otherwise, only if `Forms` table is empty, hashes a default password (`"12345678"`) and calls `DemoDataGenerator.SeedAsync`. Called once at application startup (from `Web.Api`, outside this layer). |
| `DemoDataGenerator.cs` | ~620 lines. Builds a full realistic dataset for a Belarusian telecom college (BGAS — Белорусская государственная академия связи): 9 departments, 22 teachers, 3 specialities, 5 specializations, 18 disciplines (department-templated), ~26 users (1 admin, 22 student-group users, up to 9 staff, 3 employers via `User.CreateEmployer`), 4 demo `Form`s (a discipline-satisfaction form with `WeightedRating` questions, a practice-supervisor form with `Number`/`Text` questions filled by students, an employer-satisfaction form with `TargetRole: UserRole.Employer` filled by the demo employer accounts, and a staff-satisfaction form — "Оценка удовлетворённости персонала работой в колледже" — with `TargetRole: UserRole.Staff` and `RequiredFilters: [FilterField.EmployeeCategory]`), and 305 demo `Submission`s (140 discipline-form + 90 practice-form + 15 employer-form + 60 staff-form) with per-department score baselines (`GetDepartmentScore`) and randomized realistic noise (`RandomNumberGenerator`-based, not `System.Random`). Staff-form submissions cycle deterministically through all four `EmployeeCategories` (АУП/ППС/УВП/ПОП) via `index % 4`, set with `submission.UpdateContext(submission.Context with { EmployeeCategory = ... })` since `Submission.Create` itself has no `employeeCategory` parameter (mirrors the pattern in `CreateSubmissionCommandHandler`). Uses reflection (`typeof(Submission).GetProperty(...).SetValue(...)`) to backdate `Submission.SubmittedAt` since the domain has no public setter for it — a workaround, not a general pattern to copy elsewhere. |
...
- `DemoDataGenerator` never runs when `ASPNETCORE_ENVIRONMENT=Production` (see `DbInitializer`), and otherwise runs only on an empty `Forms` table, so re-running it after any manual data entry is a no-op by design — to force a re-seed in dev, truncate/drop the DB (or at least the `forms` table) first.
