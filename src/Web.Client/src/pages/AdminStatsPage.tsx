...
export const AdminStatsPage = () => {
  ...
  if (!form || !stats)
    return <div className="p-10 text-center text-accent">Ошибка загрузки</div>;
  ...
  return (
    ...
    >
      {/* KPI Карточки (Изменили высоту на h-auto и добавили min-h) */}
      ...
      {/* Блок Графика */}
      <div className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-slate-200 mb-8 overflow-hidden">
        <h3 className="text-base md:text-lg font-bold text-slate-900 mb-6">
          Распределение оценок по вопросам
        </h3>
        {/* Для мобилок иногда требуется чуть больше места по оси Y, поэтому делаем min-h */}
              ...
              <Bar dataKey="score" radius={[4, 4, 0, 0]} maxBarSize={40}>
                {chartData.map((entry, index) => (
                  <Cell
                    ...
                    fill={
                      entry.score >= 8
                        ? "var(--color-primary)" // Используем новые CSS-переменные темы
                        : entry.score >= 5
                          ? "var(--color-secondary)"
                          : "var(--color-accent)"
                    }
      ...
      {/* Таблица Деталей с горизонтальным скроллом */}
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mb-8">
        ...
        {/* ВОТ ЗДЕСЬ МАГИЯ ДЛЯ МОБИЛОК: overflow-x-auto позволяет таблице скроллиться */}
