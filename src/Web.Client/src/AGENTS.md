...
| Directory | Purpose |
|-----------|---------|
| `contexts/` | `admin-page-context.tsx` — React context (`AdminPageProvider`/`useAdminPage`) holding a mutable `{title, subtitle, actions}` config object that `AdminLayout`'s header reads, so any admin page can push its own breadcrumb/title/action button without prop-drilling |
| `hooks/` | `use-admin-page-config.ts` (wraps `useAdminPage` in a `useEffect` that sets config on mount and clears it on unmount — call once per admin page); `use-mobile.ts` (`useIsMobile()`, a 768px `matchMedia` breakpoint hook used by the shadcn sidebar) |
