...
- **Auth model**: JWT stored in `localStorage["token"]`; role and user id are decoded client-side from the JWT payload (see `src/utils/auth.ts`). There is no refresh-token flow — logging out (any caller) goes through `logout()` in `src/utils/auth.ts`, which clears the token and dispatches an `auth:logout` window event; `AuthSessionListener` (mounted in `App.tsx`) is the only thing that navigates, via React Router's `navigate("/login", { replace: true })` — no hard `window.location` navigation anywhere in the auth flow. A 401 anywhere except the `/users/login` request itself triggers this the same way (see `src/api.ts` response interceptor).
...
- **Routing**: `react-router-dom` v7 with `BrowserRouter`; all route definitions live in `src/App.tsx`, plus a trailing `path="*"` catch-all → `/dashboard`. Route guarding uses `<ProtectedRoute allowedRoles={[...]} />` as a layout route wrapping `<Outlet />`; unauthenticated users are redirected to `/login`, and users lacking a required role are redirected to `/dashboard`. Admin-only routes are additionally wrapped in `<AdminLayout />` (sidebar + breadcrumb chrome).
- **Known gap — no SPA fallback/rewrite at the hosting layer**: `Web.Api` never serves this frontend (no `UseStaticFiles`/`MapFallbackToFile`) — it's booted separately by `AddViteApp`, and nothing in the repo configures a rewrite (nginx `try_files`, `_redirects`, `vercel.json`, etc.) for a bare/hosted static build. This is invisible in dev (Vite's dev server serves `index.html` for unknown paths itself) but means a real *browser-issued* GET to a deep link like `/login` or `/dashboard` (typed URL, bookmark, F5) can 404 on whatever static host serves a production build. In-app navigation is unaffected — every internal transition (including logout, see `logout()`/`AuthSessionListener` above) is client-side-only and never issues such a request. Fixing the server side of this is out of scope for the auth-flow fix but should be revisited before/at deploy time.
