...
| **forms/** | | |
| `forms/create-form-page.tsx` | `/admin/create-form` | `CreateFormPage` — the form builder: title textarea, toggleable required-filter buttons (`Teacher`/`Discipline`/`Department`/`Speciality`), add/reorder (buttons + native HTML5 drag-and-drop)/remove questions (`Text`/`Number`/`WeightedRating`, numeric `QuestionType` enum matching the backend), `POST /forms` on save. Uses `useAdminPageConfig` to push its title/save-button into the shared `AdminLayout` header |
